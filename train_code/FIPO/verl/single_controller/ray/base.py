...
class RayClassWithInitArgs(ClassWithInitArgs):
    ...
    def __call__(
        ...
        """
        if sharing_with is not None:  # 资源共享模式
            target_node_id = ray.get(
                sharing_with.get_node_id.remote()
            )  # 获取 sharing_with Actor 所在节点的 ID。
            visible_devices = ray.get(
                sharing_with.get_cuda_visible_devices.remote()
            )  # 获取该 Actor 可见的 CUDA 设备列表
            options = {
                "scheduling_strategy": NodeAffinitySchedulingStrategy(
                    node_id=target_node_id, soft=False
                )  # 使用 NodeAffinitySchedulingStrategy 创建一个调度策略，并将 node_id 设置为目标节点的 ID。soft=False 表示这是一个硬性要求，新 Actor 必须被调度到该节点上。
            }
