...
@dataclass
class ResourcePoolManager:  # 资源池管理
    """
    Define a resource pool specification. Resource pool will be initialized first.  定义了一个资源池
    """

    resource_pool_spec: dict[str, list[int]]  # 资源池
    mapping: dict[Role, str]  # 映射
    resource_pool_dict: dict[str, RayResourcePool] = field(default_factory=dict)

    def create_resource_pool(self):  # 创建资源池
...
class RayPPOTrainer:
    ...
    """
    在训练循环开始前（即 self.global_steps = 0 时）调用 self._validate()，通常被称为 “建立基线（Baseline）”。
    
    作用：安全气囊。如果在第 0 步验证就报错（例如维度不匹配、数据处理错误），程序会立刻崩溃，而不是等到训练了 1 小时后才报错。
    代码体现：assert val_metrics, f"{val_metrics=}"。这行断言确保了验证函数返回了有效数据，训练才能继续。
    """
    ...
    def init_workers(self):
        """Initialize distributed training workers using Ray backend.
         使用ray后端初始化分布式训练workers

        Creates:
        1. Ray resource pools from configuration 根据配置创建ray资源池
        2. Worker groups for each role (actor, critic, etc.) 为每个角色（actor，critic等）创建worker组
        """
        ...
        wg_kwargs = (
            {}
        )  # Setting up kwargs for RayWorkerGroup  设置RayWorkerGroup的kwargs
        if (
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get(
            ...
            """
            这行代码会打印出类似 Initial validation metrics: {'reward': 2.1}' 的信息，告诉你“哦，原来现在的模型水平是 2.1 分”。
            """
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch = gen_batch.repeat(
                    ...
                    interleave=True,
                )  # repeat 操作意味着对同一个 Prompt 生成 n 个不同的回答（Response），这是为了增加采样多样性。

                is_last_step = self.global_steps >= self.total_training_steps

                with marked_timer("step", timing_raw):
                    # generate a batch 根据gen_batch生成完整的文本序列
                    with marked_timer("gen", timing_raw, color="red"):
                        if not self.async_rollout_mode:
                            gen_batch_output = self.actor_rollout_wg.generate_sequences(
                                gen_batch
                            )  # Actor 模型根据 Prompt 生成完整的文本序列
                        else:
                            gen_batch_output = (
