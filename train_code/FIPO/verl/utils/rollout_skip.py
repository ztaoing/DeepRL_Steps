...
class RolloutSkip:
    ...
    def __init__(self, config, rollout_wg):
        ...
        self.gbs = int(
            config.data.get("gen_batch_size", config.data.get("train_batch_size", 0))
        )

        self.dumped_dir = Path(
            self.rollout_config.get("skip_dump_dir", "/tmp/verl/rollout_dump")
        )
    ...
    def curr_path_dump(self):
        return self.dumped_dir.joinpath(
            f"{self.exp_name}_{self.project_name}_GBS{self.gbs}__N{self.n}"
        ).absolute()

    """
    调用 wrap 方法。这通常意味着它会替换或装饰原有的 generate_sequences 函数。当后续代码调用生成指令时，
    不会执行原本的“从头生成”，而是执行 RolloutSkip 中定义的特殊逻辑（例如直接返回缓存数据，或仅生成部分 token）。
    """

    def wrap_generate_sequences(self):
        try:
            self._rollout_wg.generate_sequences = wrap_generate_sequences(
                self, self._rollout_wg
            )
            print(
